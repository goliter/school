<template>
  <div class="personal-info-container">
    ...
    <div v-else-if="teacher" class="personal-info-content">
    ...
    <div v-else class="error-state">
      <p>加载个人信息失败，请稍后重试</p>
      <button class="btn btn-primary" @click="fetchTeacherInfo">
        重新加载
      </button>
    </div>
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          <form @submit.prevent="saveEdit">
            ...
            <div class="form-group">
              <label>职责</label>
              <input v-model="editForm.duty" type="text" class="form-control" />
            </div>
...
const editForm = ref<Teacher & { password?: string }>({
  teacherId: "",
  name: "",
  title: "",
  office: "",
...
const fetchTeacherInfo = async () => {
  try {
    ...
    const currentTeacher = allTeachers.find((t) => t.userId === userId);

    if (currentTeacher) {
      teacher.value = currentTeacher;
    } else {
...
const currentMajor = computed(() => {
  if (teacher.value) {
    return majors.value.find(
      (major) => major.majorCode === teacher.value?.majorCode
    );
  }
  return undefined;
});
...
const saveEdit = async () => {
  try {
    ...
    const teacherData = {
      ...
      office: editForm.value.office,
      phone: editForm.value.phone,
      duty: editForm.value.duty,
      majorCode: editForm.value.majorCode,
      userId: currentUser.value?.userId || editForm.value.userId || "",
      password: editForm.value.password, // 直接使用表单中的密码
      status: currentUser.value?.status || 1, // 默认状态为1(正常)
    };
    await teacherApi.updateTeacher(editForm.value.teacherId, teacherData);
...
// 初始化数据
onMounted(async () => {
  await Promise.all([fetchTeacherInfo(), fetchMajors(), fetchColleges()]);
});
</script>
