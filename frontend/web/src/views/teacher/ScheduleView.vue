...
const loadTeacherSchedules = async () => {
  try {
    ...
    // 获取当前教师信息
    const teacher = currentUser.value.teacher;
    //console.log("111",teacher);
    if (!teacher) {
      console.error("无法获取当前教师信息");
      return;
    }

    // 调用后端API获取教师的课程安排
    const schedules = await scheduleApi.getTeacherSchedules(teacher.teacherId);

    // 处理返回的课程安排数据
    courseSchedules.value = schedules
      .map((schedule: any) => {
        // 将数据库中的scheduleInfo转换为课程安排
        const schedules: CourseSchedule[] = [];
        let scheduleInfo = {};
        try {
          scheduleInfo = JSON.parse(schedule.scheduleInfo || "{}");
        } catch (e) {
          console.error("解析scheduleInfo失败:", e);
        }
